...
def conv2d(x, inputFeatures, outputFeatures, name):
    with tf.variable_scope(name):
        ...
        saver = tf.train.Saver({name + "w": w, name + "b": b})
        return conv, saver
...
def dense(x, inputFeatures, outputFeatures, scope=None, with_w=False):
    with tf.variable_scope(scope or "Linear"):
        ...
        saver = tf.train.Saver({scope + "W": matrix, scope + "b": bias})
        if with_w:
            return tf.matmul(x, matrix) + bias, matrix, bias, saver
        else:
            return tf.matmul(x, matrix) + bias, saver
